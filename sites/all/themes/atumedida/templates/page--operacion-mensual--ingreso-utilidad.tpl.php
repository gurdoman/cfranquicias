...
(function($){
    ...
    var drawChartComparativo = function (objeto, contenedor) {   
        ...
        var venta = parseFloat(0);
        var gasto = parseFloat(0);
        var data = [['Mes', 'Ingreso', 'Gasto', 'U/Ing']];

        for (var key in objeto) {
            if (objeto.hasOwnProperty(key)) {
                var diferencia = parseFloat(objeto[key].vn) - parseFloat(objeto[key].gastos);
                data.push([objeto[key].mes, parseFloat(objeto[key].vn),parseFloat(objeto[key].gastos),diferencia]);
                venta = venta + parseFloat(objeto[key].vn);
                gasto = gasto + parseFloat(objeto[key].gastos);
            }
        }
        var diferencia = venta-gasto;
        //data.push(['Total',venta, gasto, diferencia]);

        var options = {
                title: 'Utilidad Neta vs Ingresos',
        ...
        var chart = new google.visualization.ComboChart(document.getElementById(contenedor));
        ...
        google.visualization.events.addListener(table, 'ready', function () {
            var a         = document.createElement('a');
                ...
                a.target      = '_blank';
                a.download    = 'Utilidad_Neta_vs_Ingresos.csv';
    ...
    var drawChartComparativo2 = function (objeto, contenedor) {   
        ...
        for (var key in objeto) {
            if (objeto.hasOwnProperty(key)) {
                var diferencia = parseFloat(objeto[key].vb) - parseFloat(objeto[key].gastos);
                data.push([objeto[key].mes, parseFloat(objeto[key].vb),parseFloat(objeto[key].gastos),diferencia]);
                venta = venta + parseFloat(objeto[key].vb);
                gasto = gasto + parseFloat(objeto[key].gastos);
            }
        }
        var diferencia = venta-gasto;
        ...
        var options = {
                title: 'Utilidad Bruta vs Ingresos',
                colors: ['blue', 'orange', 'green'],
                seriesType: 'bars',
                series: {2: {type: 'line'}}
          };
        ...
        google.visualization.events.addListener(table, 'ready', function () {
            var a         = document.createElement('a');
                a.href        = 'data:attachment/csv,' + csv;
                a.target      = '_blank';
                a.download    = 'Utilidad_Bruta_vs_Ingresos.csv';
                a.setAttribute('class','links-graficas');
                a.innerHTML = "Obtener csv";
                $div2.empty();
                $div2.append("<i class='fa fa-table'></i>",a);
    ...
    $(document).ready(function(){
        ...
        drawChartComparativo(c1, "chartComparativo");
        drawChartComparativo2(c1, "chartComparativo2");
